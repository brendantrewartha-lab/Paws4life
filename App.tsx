...
const App: React.FC = () => {
  ...
  const handleAddRecord = () => {
    ...
    setFormDog(prev => ({ 
      ...prev, 
      healthRecords: [...(prev.healthRecords || []), rec], 
      reminders: [...(prev.reminders || []), rem] 
    }));
    setRecordModal(null);
  };
    ...
    <div className="flex flex-col h-screen max-w-xl mx-auto bg-slate-50 relative shadow-2xl overflow-hidden">
      ...
      {view === 'profiles' && (
        <div className="fixed inset-0 z-[200] bg-white flex flex-col animate-in">
          ...
          <div className="flex-1 p-6 space-y-4 overflow-y-auto bg-slate-50">
            <button onClick={() => { setFormDog({ healthRecords: [], reminders: [] }); setView('edit-form'); }} className="w-full py-6 border-4 border-dashed border-orange-200 bg-white text-orange-600 rounded-[2.5rem] font-black uppercase text-xs tracking-[0.2em] shadow-sm hover:bg-orange-50 transition-all">Add New Companion</button>
            {profiles.map(p => (
              <div key={p.id} onClick={() => { setViewId(p.id); setFormDog(p); setView('profile-detail'); }} className="p-4 bg-white border border-slate-100 rounded-[2rem] flex items-center gap-4 cursor-pointer hover:shadow-md transition-all">
                ...
                {activeId === p.id && <span className="bg-orange-500 text-white text-[8px] px-2.5 py-1 rounded-full font-black uppercase tracking-widest shadow-sm">Active</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {view === 'profile-detail' && formDog && (
        <div className="fixed inset-0 z-[220] bg-white flex flex-col animate-in">
          <header className="bg-orange-600 text-white p-4 pt-12 flex items-center justify-between shadow-md">
            ...
            <button onClick={() => setView('edit-form')} className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center"><i className="fa-solid fa-pen-to-square"></i></button>
          </header>
          <div className="flex-1 p-6 space-y-8 overflow-y-auto bg-slate-50">
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <button onClick={() => { setActiveId(formDog.id!); setView('chat'); }} className={`mt-4 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm transition-all active:scale-95 ${activeId === formDog.id ? 'bg-slate-900 text-white' : 'bg-white text-slate-400 border'}`}>
                  {activeId === formDog.id ? 'Main Assistant Active' : 'Select Assistant'}
                </button>
              </div>
            </div>
      ...
      {view === 'settings' && (
        <div className="fixed inset-0 z-[230] bg-white flex flex-col animate-in">
          <header className="bg-orange-600 text-white p-4 pt-12 flex items-center gap-3">
            <button onClick={() => setView('chat')} className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center"><i className="fa-solid fa-chevron-left"></i></button>
            <h2 className="text-xl font-black italic">Account & Profile</h2>
          </header>
      ...
      {view === 'admin' && (
        <div className="fixed inset-0 z-[300] bg-white flex flex-col animate-in">
          <header className="bg-slate-900 text-white p-4 pt-12 flex items-center gap-3">
            <button onClick={() => setView('settings')} className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center"><i className="fa-solid fa-chevron-left"></i></button>
            ...
            {getFilteredReminders().length === 0 ? <div className="text-center py-20 text-slate-300 italic opacity-50"><i className="fa-solid fa-bell-slash text-4xl mb-4 block"></i> No pending reminders</div> : 
              getFilteredReminders().map(r => (
                <div key={r.id} className="p-5 bg-white border border-slate-100 rounded-[2rem] flex items-center gap-4 shadow-sm">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-black text-slate-800 leading-tight mb-1">{r.title}</div>
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{r.dog} • {r.type} Schedule</div>
                  </div>
                  <div className="text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-1.5 rounded-xl border border-orange-100">{r.date}</div>
                </div>
              ))
            }
          </div>
